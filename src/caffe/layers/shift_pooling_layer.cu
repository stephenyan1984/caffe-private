  ...
  CUDA_KERNEL_LOOP(index, nthreads)
  {
    ...
    for (int sy = 0; sy < stride_h; ++sy) {
      for (int sx = 0; sx < stride_w; ++sx) {
        ...
        const Dtype *top_diff_ptr = top_diff + offset;
        if (mask) {
          ...
          const Dtype *top_mask_ptr = top_mask + offset;
          for (int ph = phstart; ph < phend; ++ph) {
            for (int pw = pwstart; pw < pwend; ++pw) {
              if (top_mask_ptr[ph * pooled_width + pw] == h * width + w) {
      ...
      for (int sx = 0; sx < stride_w; ++sx) {
        ...
        for (int ph = phstart; ph < phend; ++ph) {
          for (int pw = pwstart; pw < pwend; ++pw) {
            ...
            int wend = min(wstart + kernel_w, width + pad_w);
            int pool_size = (hend - hstart) * (wend - wstart);
            gradient += top_diff_ptr[ph * pooled_width + pw] / pool_size;
          }
        }
      }
    }
