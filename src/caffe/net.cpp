...
void Net<Dtype>::BackwardFromTo(int start, int end) {
  ...
  for (int i = start; i >= end; --i) {
    if (layer_need_backward_[i]) {
      DLOG(ERROR)<< "Backward pass " << layer_names_[i];
...
void Net<Dtype>::CopyTrainedLayersFrom(const NetParameter& param) {
  ...
  for (int i = 0; i < num_source_layers; ++i) {
    ...
    bool strict_dim = target_layer->layer_param().strict_dim();
    for (int j = 0; j < target_blobs.size(); ++j) {
      if (strict_dim) {
        target_blobs[j]->FromProto(source_layer.blobs(j), false);
      } else {
        int source_blob_count = 1;
        BlobShape source_blob_shape = source_layer.blobs(j).shape();
        if(source_blob_shape.dim_size() > 0) {
          for (int k = 0; k < source_blob_shape.dim_size(); ++k) {
            source_blob_count *= source_blob_shape.dim(k);
          }
        } else {
          source_blob_count = source_layer.blobs(j).num() * source_layer.blobs(j).channels() *
              source_layer.blobs(j).height() * source_layer.blobs(j).width();
        }
        CHECK_EQ(target_blobs[j]->count(), source_blob_count);
        BlobProto blob_copy = source_layer.blobs(j);
        blob_copy.clear_num();
        blob_copy.clear_channels();
        blob_copy.clear_height();
        blob_copy.clear_width();
        blob_copy.mutable_shape()->clear_dim();
        for (int k = 0; k < target_blobs[j]->num_axes(); ++k) {
          blob_copy.mutable_shape()->add_dim(target_blobs[j]->shape(k));
        }
        target_blobs[j]->FromProto(blob_copy, true);
      }
    }
  }
}
